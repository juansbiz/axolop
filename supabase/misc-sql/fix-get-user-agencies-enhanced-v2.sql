...
-- This fixes the column and table name issues
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        CASE 
            ...
            WHEN ai.user_id IS NOT NULL THEN 'invited'
            ELSE NULL
        END as user_role,
        CASE 
        ...
        COALESCE(am.joined_at, ai.created_at) as joined_at
    FROM public.agencies a
    LEFT JOIN public.agency_members am ON a.id = am.agency_id AND am.user_id = p_user_id
    LEFT JOIN public.agency_invites ai ON a.id = ai.agency_id AND ai.user_id = p_user_id AND ai.status IN ('pending', 'accepted')
    WHERE (
